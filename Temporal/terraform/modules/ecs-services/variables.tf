...
variable "worker_min_capacity" {
  description = "Minimum number of worker tasks for ECS Application Auto Scaling. Use 2+ to survive rolling deployments."
  type        = number
  default     = 2
}

variable "worker_max_capacity" {
  description = "Maximum number of worker tasks ECS Application Auto Scaling may launch."
  type        = number
  default     = 10
}

variable "worker_service_registry_arn" {
  description = "Cloud Map service discovery ARN the worker (awsvpc) registers each task in, so Prometheus discovers worker metrics endpoints via dns_sd."
  type        = string
}

variable "server_service_registry_arn" {
  description = "Cloud Map service discovery ARN the server (awsvpc) registers in, so Prometheus discovers server metrics on port 8001 via dns_sd."
  type        = string
}

variable "worker_service_urls" {
  description = "Map of activity env var name → dependency service URL (e.g. FRAUD_SERVICE_URL → http://fraud.<ns>:8000). Injected into the worker environment."
  type        = map(string)
  default     = {}
}
