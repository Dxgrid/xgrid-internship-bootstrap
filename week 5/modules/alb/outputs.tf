...
output "grafana_target_group_arn" {
  description = "ARN of the Grafana ALB target group — used by the prometheus module to register the monitoring EC2."
  value       = aws_lb_target_group.grafana.arn
}

output "grafana_tg_arn_suffix" {
  description = "Grafana target group ARN suffix for CloudWatch alarm dimensions."
