...
output "grafana_url" {
  description = "Grafana dashboard URL via ALB path routing — open this in your browser after deploy."
  value       = "http://${module.alb.alb_dns_name}/grafana"
}

output "prometheus_direct_url" {
  description = "Direct Prometheus UI URL (bypasses ALB — for development debugging only)."
  value       = module.prometheus.prometheus_direct_url
}

output "monitoring_ec2_public_ip" {
  description = "Public IP of the monitoring EC2 — use this with SSM Session Manager for debugging."
  value       = module.prometheus.monitoring_ec2_public_ip
}

output "grafana_alarm_name" {
  description = "CloudWatch alarm that fires when Grafana is unreachable."
