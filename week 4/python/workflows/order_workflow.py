...
class OrderWorkflow:
    ...
    async def run(self, order: OrderInput) -> OrderOutput:
        try:
            ...
            # Register compensation to refund customer if shipping fails
            ...
            self.shipped_items = list(shipped_results)
            if self._cancelled: return await self._early_exit()
    ...
    async def _run_compensations(self) -> None:
        """Run all registered compensations in reverse order (LIFO) with robust error handling."""
        workflow.logger.info("Running saga compensations (%d actions)", len(self._compensations))
        # Iterate through all compensations in reverse order
        for compensation in reversed(self._compensations):
            try:
                workflow.logger.info("Running compensation activity: %s", compensation["activity"].__name__)
                await workflow.execute_activity(
                    compensation["activity"],
                    compensation["input"],
                    start_to_close_timeout=timedelta(seconds=10),
                )
                workflow.logger.info("Compensation succeeded: %s", compensation["activity"].__name__)
            except Exception as e:
                # Log but CONTINUE — don't block other compensations
                workflow.logger.error(
                    "Compensation failed for %s: %s — continuing with remaining compensations",
                    compensation["activity"].__name__, str(e)
                )
    ...
    @workflow.signal(name="update_address")
