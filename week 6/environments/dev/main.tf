...
module "efs" {
  ...
  # prometheus_task_role_arn and grafana_task_role_arn intentionally omitted to avoid a
  # circular dependency (prometheus/grafana depend on efs, efs cannot depend back on them).
  # EFS access is controlled by the IAM identity policies on the task roles instead.
}
...
module "ecs" {
  ...
  # app_image defaults to the ECR repo created by this module tagged :latest.
  # Push the image before applying to avoid ECS task start failures.
}
...
module "prometheus" {
  ...
  cloudmap_namespace_arn     = module.ecs.cloudmap_namespace_arn
  cloudmap_namespace_name    = module.ecs.cloudmap_namespace_name
  demo_app_dns_name          = module.ecs.demo_app_cloudmap_dns_name
...
module "grafana" {
  ...
  db_master_username       = module.secrets.db_username
